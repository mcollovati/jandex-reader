...
@Command(name = "annotated", mixinStandardHelpOptions = true,
...
public class AnnotatedCommand extends IndexCommand {
    ...
    public enum Target { CLASS, FIELD, METHOD, PARAMETER, RECORD_COMPONENT, TYPE_USE }

    @Parameters(index = "0", paramLabel = "<annotation>",
            description = "Annotation name, fully qualified or simple (e.g. 'Singleton' or '@jakarta.inject.Singleton').")
    ...
    @Option(names = {"-t", "--target"}, paramLabel = "<target>", split = ",", converter = TargetConverter.class,
            description = "Only these target kinds: class, field, method, parameter, record-component, type-use.")
    List<Target> targets;

    @Option(names = {"-c", "--classes"},
    ...
    protected int execute(List<LoadedIndex> indexes) throws Exception {
        ...
        if (classesOnly) {
            ...
            instances.stream().map(a -> declaringClass(a.target())).filter(Objects::nonNull).forEach(classes::add);
            if (json) {
                printJson(classes);
            } else {
            ...
            for (AnnotationInstance instance : instances) {
                ...
                table.row(Format.targetKind(instance.target()), Format.target(instance.target()),
    ...
    private static String declaringClass(AnnotationTarget target) {
        ...
        return switch (target.kind()) {
            ...
            case METHOD_PARAMETER -> target.asMethodParameter().method().declaringClass().name().toString();
            case RECORD_COMPONENT -> target.asRecordComponent().declaringClass().name().toString();
